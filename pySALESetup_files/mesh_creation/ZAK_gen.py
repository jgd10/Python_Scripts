...
c = 0
for vlim in np.linspace(0,1,NN):
    # Wipe materials and mesh for next loop
    pss.clear_mesh()
    ...
    if abs(vol_frac_calc - vol_frac) <= 0.02:
        ...
        P     = 5
        K_[c] = pss.max_porosity_variation(partitions=P)
        Z_[c],A_[c],F = pss.Fabric_Tensor_disks(xcr,ycr,radii,tolerance=1.e-6)
...
ax1.plot(K_,Z_,marker='o',mfc='None',linestyle=' ',color='k')
ax2.plot(K_,A_,marker='D',mfc='None',linestyle=' ',color='k')
ax3.plot(Z_,abs(A_),marker='s',mfc='None',linestyle=' ',color='k')
ax4.scatter(K_,Z_,c=A_,cmap='coolwarm')
...
fig.tight_layout()
fig.savefig('ZAK_variation.pdf',dpi=300)
