...
Sip_c = [17.6e-3,2.6e-3+c]
Sip_d = [17.6e-3,2.6e-3]
Sip_ = np.row_stack((Sip_a,Sip_b,Sip_c,Sip_d))
...
pss.generate_mesh(L_cells,T_cells,mat_no=4,GridSpc=GRIDSPC)
mats = pss.mats
...
pss.fill_polygon(Sip_[:,0],Sip_[:,1],int(mats[2]))
pss.fill_polygon(PMMA_[:,0],PMMA_[:,1],int(mats[3]))
...
pss.save_general_mesh(fname='meso_m_Sip4_{}x{}.iSALE'.format(T_cells,L_cells),mixed=False,noVel=True)
plt.show()
