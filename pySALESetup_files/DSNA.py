...
for AA in A_:
    for SS in S_:
        ...
        b     = a*np.sqrt(1.-SS**2.)                    # S, equivalent to eccentricity. S = 0 => circle
        ...
        for item in range(N):
            ...
            rho[item] = np.random.normal(rho[item],AA*rho[item])    # Randomly select a radius, with the radius of the ellipse as a mean and A*radius as the std
